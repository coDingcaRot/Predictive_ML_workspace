...
def ex6_grid_searching_MLP_regs():
    ...
    scY = StandardScaler() #Chat fixed
    trainY_scaled = scY.fit_transform(np.array(trainY).reshape(-1, 1)).ravel()
    testY_scaled = scY.transform(np.array(testY).reshape(-1, 1)).ravel()
    valY_scaled = scY.transform(np.array(valY).reshape(-1, 1)).ravel()

    # Build basic multilayer perceptron.
    model1 = MLPRegressor(
        # 3 hidden layers with 150 neurons, 100, and 50.
        hidden_layer_sizes=(150, 100, 50),
        max_iter=50,  # epochs -> increased it. as it could not reach convergence
        activation='relu',
        solver='adam',  # optimizer
        verbose=1)
    model1.fit(trainX_scaled, trainY_scaled)
    ...
    param_grid = {
        'hidden_layer_sizes': [(150, 100, 50), (120, 80, 40), (100, 50, 30)],
        'max_iter': [50, 100], # changed it to a singular iteration.
        'activation': ['tanh', 'relu'],
        'solver': ['sgd', 'adam'],
        'alpha': [0.0001, 0.05],
        'learning_rate': ['constant', 'adaptive'],
    ...
    model2 = MLPRegressor()
    gridModel = RandomizedSearchCV(model2, param_grid, n_jobs=-1, cv=10, scoring='neg_mean_squared_error')
