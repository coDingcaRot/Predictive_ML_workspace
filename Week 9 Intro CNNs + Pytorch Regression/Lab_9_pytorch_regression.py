...
class MyNeuralNet(nn.Module):
    def __init__(self, num_units=10, nonlin=F.relu):
        ...
        self.dense0 = nn.Linear(4, num_units)  # Input layer: 4 inputs -> num_units
        self.dense1 = nn.Linear(num_units, 15)  # Additional hidden layer: num_units -> 15
        self.dense2 = nn.Linear(15, 10)  # Hidden layer: 15 -> 10
        self.output = nn.Linear(10, 1)  # Output layer: 10 -> 1

    def forward(self, X, **kwargs):
        X = self.nonlin(self.dense0(X))  # Apply nonlin (ReLU) to first layer
        X = F.relu(self.dense1(X))  # Apply ReLU to the new hidden layer
        X = F.relu(self.dense2(X))  # Apply ReLU to the second hidden layer
...
def buildModel(x, y):
    ...
    params = {
        ...
        'optimizer': optim.SGD,
        'optimizer__momentum': 0.9,  # Standard default. Range is 0 to 1.
    }
